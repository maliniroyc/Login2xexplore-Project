...
function validateData() {
   ...
   ship_no = $('#ship-no').val();
    ...
    if (description === '') {
          alert("Shipping Description is MISSING");
        $('#description').focus();
        return "";
    }
...
function changeData() {
    ...
    dbName, 
    relName, 
    localStorage.getItem('shiprecno')
            );
    jQuery.ajaxSetup({
        async: false
    });
    var resJsonObj = executeCommandAtGivenBaseUrl(
            updateRequest.baseURL, 
    jpdbIML
            );
    jQuery.ajaxSetup({
        async: true
...
var token = "90932135|-31949220626550404|90963714";
